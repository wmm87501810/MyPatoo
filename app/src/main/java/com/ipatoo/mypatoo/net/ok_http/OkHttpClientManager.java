...
import com.ipatoo.mypatoo.utils.LogUtils;
...
public class OkHttpClientManager {
    ...
    public void execute(Request request, ResultCallback callback) {
        ...
        LogUtils.i("OkHttpRequest", request != null ? request.toString() : "");
        ...
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(final Response response) {
                try {
                    ...
                    LogUtils.d("OKHttpResponse", string);
                    if (response.code() != 200) {
                        JSONObject jsonObject = new JSONObject(string);
                        String str = jsonObject.getString("description");
                        Exception exception;
                        if (!TextUtils.isEmpty(str)) {
                            exception = new ErrorException(str);
                        } else {
                            exception = new Exception();
                        }
                        sendFailResultCallback(response.request(), exception, resCallBack);
                    } else {
